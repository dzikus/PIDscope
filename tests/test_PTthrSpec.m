% test_PTthrSpec.m - Tests for throttle x frequency spectrogram
% NOTE: PTthrSpec expects X,Y as column vectors, F in kHz
...
%! [freq, ampMat] = PTthrSpec(throttle, signal, Fs_khz, 0);
...
%! throttle = ones(N, 1) * 50;
%! signal = randn(N, 1);
%! [~, ampMat] = PTthrSpec(throttle, signal, Fs_khz, 0);
