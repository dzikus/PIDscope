function [headerFile, csvFile] = PTquicJson2csv(jsonFile)
%% PTquicJson2csv - Convert QuickSilver JSON blackbox export to CSV
%
% Reads JSON exported by BossHobby Configurator ("QUIC download") and
% produces CSV + synthetic header compatible with PTimport/PTload pipeline.
%
% Unit conversions applied:
%   gyro/setpoint RPY: rad/s -> deg/s (multiply by 180/pi)
%   yaw: negated (QS convention opposite to BF)
%   motor: 0-1 range -> 0-2000 (BF scale, PTload does /2000*100)
%   PID terms: divided by 1000 (stored as int16*1000)
%   throttle setpoint: 0-1 -> 0-1000 (BF scale)
%
% [headerFile, csvFile] = PTquicJson2csv(jsonFile)
  ...
  % Write synthetic header file (mimics BBL header for PTimport)
