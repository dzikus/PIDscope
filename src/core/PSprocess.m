%% PSprocess - script that extracts subset of total data based on highlighted epoch in main fig 
...
set(PSfig, 'pointer', 'watch')
    if ~isempty(filenameA)
        if isempty(epoch1_A) || isempty(epoch2_A)
            ...
            guiHandles.Epoch1_A_Input = uicontrol(PSfig,'style','edit','string',[int2str(epoch1_A)],'fontsize',fontsz,'units','normalized','Position',[posInfo.Epoch1_A_Input],...
            'callback','@textinput_call; epoch1_A=str2double(get(guiHandles.Epoch1_A_Input, ''String'')); PSprocess;PSplotLogViewer;');
            guiHandles.Epoch2_A_Input = uicontrol(PSfig,'style','edit','string',[int2str(epoch2_A)],'fontsize',fontsz,'units','normalized','Position',[posInfo.Epoch2_A_Input],...
            'callback','@textinput_call;epoch2_A=str2double(get(guiHandles.Epoch2_A_Input, ''String'')); PSprocess;PSplotLogViewer;');
        end
    ...
    if ~isempty(filenameB)
         if isempty(epoch1_B) || isempty(epoch2_B)
            ...
            guiHandles.Epoch1_B_Input = uicontrol(PSfig,'style','edit','string',[int2str(epoch1_B)],'fontsize',fontsz,'units','normalized','Position',[posInfo.Epoch1_B_Input],...
             'callback','@textinput_call; epoch1_B=str2double(get(guiHandles.Epoch1_B_Input, ''String''));PSprocess;PSplotLogViewer; ');
            guiHandles.Epoch2_B_Input = uicontrol(PSfig,'style','edit','string',[int2str(epoch2_B)],'fontsize',fontsz,'units','normalized','Position',[posInfo.Epoch2_B_Input],...
             'callback','@textinput_call; epoch2_B=str2double(get(guiHandles.Epoch2_B_Input, ''String''));PSprocess;PSplotLogViewer; ');
        end
...
set(PSfig, 'pointer', 'arrow')
    ...
    errmsg.PSprocess=PSerrorMessages('PSprocess', ME);
end
