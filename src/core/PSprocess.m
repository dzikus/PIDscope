...
set(PSfig, 'pointer', 'watch')
    if ~isempty(filenameA)
        if isempty(epoch1_A) || isempty(epoch2_A)
            ...
            'callback','@textinput_call; epoch1_A=str2num(get(guiHandles.Epoch1_A_Input, ''String'')); PSprocess;PSplotLogViewer;');
            ...
            'callback','@textinput_call;epoch2_A=str2num(get(guiHandles.Epoch2_A_Input, ''String'')); PSprocess;PSplotLogViewer;');
        end
    ...
    if ~isempty(filenameB)
         if isempty(epoch1_B) || isempty(epoch2_B)
            ...
            guiHandles.Epoch1_B_Input = uicontrol(PSfig,'style','edit','string',[int2str(epoch1_B)],'fontsize',fontsz,'units','normalized','Position',[posInfo.Epoch1_B_Input],...
             'callback','@textinput_call; epoch1_B=str2num(get(guiHandles.Epoch1_B_Input, ''String''));PSprocess;PSplotLogViewer; ');
            guiHandles.Epoch2_B_Input = uicontrol(PSfig,'style','edit','string',[int2str(epoch2_B)],'fontsize',fontsz,'units','normalized','Position',[posInfo.Epoch2_B_Input],...
             'callback','@textinput_call; epoch2_B=str2num(get(guiHandles.Epoch2_B_Input, ''String''));PSprocess;PSplotLogViewer; ');
        end
