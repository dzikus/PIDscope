function [Tm freq specMat] = PStimeFreqCalc(Y, F, smoothFactor, subsampleFactor)
%% [Tm, freq, specMat] = PStimeFreqCalc(Y, F, smoothFactor)
...
% ----------------------------------------------------------------------------------
    ...
    specMat=[];freq = [];
    for i = 1 : size(Yseg,1)
        [freq specMat(i,:)] = PSSpec2d(Yseg(i,:), F, 1);
