function PTplotDynNotchOverlay(ax, notchMat, xData, imgHeight, freqMax, mode)
%% PTplotDynNotchOverlay - overlay dynamic notch frequencies on spectrogram
%  ax        - axes handle
...
if strcmp(mode, 'throttle')
    % Average notch freq per throttle bin (1-100, same as PTthrSpec)
    for n = 1:numNotches
        xPts = [];
        yPts = [];
        for bin = 1:100
